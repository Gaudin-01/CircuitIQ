  ...
  handler: async (ctx): Promise<LeaderboardEntry[]> => {
    ...
    for (const [userId, attempts] of userAttempts) {
      ...
      entries.push({
        ...
        name: user.name && user.name !== "unset" ? user.name : "Anonymous",
...
//   args: {},
//   handler: async (ctx): Promise<LeaderboardEntry[]> => {
//     // Fetch all quiz attempts (limited to last 1000 for performance)
//     const allAttempts = await ctx.db
//       .query("quizAttempts")
//       .order("desc")
//       .take(1000);
...
//           entries.push({
//               userId,
//               name: user.name && user.name !== "unset" ? user.name : "New User",
//               quizzesTaken: attempts.length,
//               totalCorrect,
//               totalQuestions,
//               avgPercentage,
...
//     // Sort by avg percentage desc, then by quizzes taken desc as tiebreaker
//     entries.sort((a, b) => {
//       if (b.avgPercentage !== a.avgPercentage) {
//         return b.avgPercentage - a.avgPercentage;
//       }
